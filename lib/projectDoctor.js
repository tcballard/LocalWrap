...
// Definitions shared with the renderer live in public/shared-constants.js so
// the two sides of the IPC bridge cannot drift.
const {
  DOCTOR_CHECKS: CHECK_DEFINITIONS,
  DOCTOR_ACTIONS: ACTIONS,
  DOCTOR_ACTION_LABELS: ACTION_LABELS,
  DOCTOR_MUTATING_ACTIONS,
} = require('../public/shared-constants');
...
const MUTATING_ACTIONS = new Set(DOCTOR_MUTATING_ACTIONS);
...
function getDependencyWarning(cwd, fsImpl = fs) {
  ...
  return {
    ...
    message: `Dependencies may be missing. Run ${installCommand} in this folder if start fails.`,
    installCommand,
  };
}
...
async function diagnoseProjectDraft(draft = {}, options = {}) {
  ...
  if (validation.errors.length > 0) {
    ...
    return {
      ...finalizeDiagnosis(diagnosis, 'Fix Doctor errors before starting.', 'failed', { now }),
      validation,
    };
  }
  ...
  if (warningCount > 0) {
    ...
    return {
      ...finalizeDiagnosis(
        diagnosis,
        'Project can start, but Doctor found warnings.',
        'attention',
        {
          now,
        }
  ...
  return {
    ...finalizeDiagnosis(diagnosis, 'Project looks ready to start.', 'idle', { now }),
    validation,
  };
}
