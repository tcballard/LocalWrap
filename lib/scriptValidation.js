 ...
 */
const ALLOWED_COMMANDS = ['npm', 'npx', 'yarn', 'pnpm', 'node', 'bun', 'python', 'python3', 'deno'];
...
// defense-in-depth (and because the Windows path may use a shell). `%` and `^`
// matter on Windows specifically: cmd.exe expands %VAR% and treats ^ as its
// escape character; quotes are rejected so no quoting tricks survive either.
